...
class Args:
    include_bcr = True
    bcr_file = "raw/iedb/bcr_singlechain_vh.tsv"  # canonical input
    top_labels = 10
    min_label_count = 14
    random_state = 42
    human_only = True
...
def replicate_bcr_pilot_df(df, spec, args, seed):
    ...
    work = work[work["sequence"].map(is_legit_sequence)]  # match build_pilot_slice filter
...
PERQUERY = []      # per-query rows across seeds (seed,label,shm,blosum_correct,esm_correct)
...
for s in seeds:
    ...
    # Combine predictions and SHM (+ antigen label + seed) for within-antigen analysis
    test_results = pd.DataFrame({
        "seed": s,
        "label": pilot_df.loc[test_idx, "label"].values,
        "original_index": test_original_idx,
        "shm": test_shm,
        "blosum_correct": blosum_pred["exp_recall@1"].values,
        "esm_correct": esm_pred["exp_recall@1"].values
    })
    PERQUERY.append(test_results)
...
# ============================================================================
# 7. WITHIN-ANTIGEN SHM stratification (decouple SHM density from antigen identity)
# ============================================================================
pq = pd.concat(PERQUERY, ignore_index=True)
pq.to_csv("outputs/reports/shm_perquery_bcr_l4.csv", index=False)
print(f"\nSaved per-query SHM table ({len(pq)} rows across {pq.seed.nunique()} seeds) -> outputs/reports/shm_perquery_bcr_l4.csv")

def bin_shm(x):
    return "Low" if x < 0.05 else ("Med" if x < 0.15 else "High")
pq["shm_bin"] = pq["shm"].map(bin_shm)
pq["delta"] = pq["esm_correct"] - pq["blosum_correct"]

# Identify the HIV-1 label(s)
hiv_labels = sorted({l for l in pq.label.unique() if "hiv" in str(l).lower() or "envelope" in str(l).lower()})
print("\nHIV-1 label(s) detected:", hiv_labels)
pq["is_hiv"] = pq["label"].isin(hiv_labels)

def cell(sub):
    if len(sub) == 0: return "n=0"
    return f"n={len(sub):4d}  BLOSUM={sub.blosum_correct.mean():.3f}  ESM={sub.esm_correct.mean():.3f}  d(ESM-BL)={sub.delta.mean():+.3f}"

print("\n=== (A) WITHIN HIV-1 envelope: does the ESM advantage track SHM, with antigen held fixed? ===")
hiv = pq[pq.is_hiv]
for b in ["Low", "Med", "High"]:
    print(f"  HIV-1 {b:4s}: {cell(hiv[hiv.shm_bin==b])}")
print(f"  HIV-1 ALL : {cell(hiv)}")

print("\n=== (B) WITHIN the High-SHM bin: is the ESM advantage HIV-specific or general? ===")
hi = pq[pq.shm_bin=="High"]
print(f"  High-SHM, HIV-1     : {cell(hi[hi.is_hiv])}")
print(f"  High-SHM, non-HIV   : {cell(hi[~hi.is_hiv])}")

print("\n=== (C) Per-antigen ESM-BLOSUM delta, split by SHM bin (antigens with >=200 queries total) ===")
big = pq.groupby("label").size(); big = big[big>=200].index
for lab in sorted(big):
    s = pq[pq.label==lab]
    lo, hii = s[s.shm_bin=="Low"], s[s.shm_bin=="High"]
    print(f"  {lab[:34]:34s} | Low {cell(lo)} | High {cell(hii)}")
